...
void run_tests() {
    ...
    for (int test = 1; test <= TEST_CASES; test++) {
        snprintf(input_path, sizeof(input_path), "src/input/input%d.txt", test);
        snprintf(output_path, sizeof(output_path), "src/output/output%d.txt", test);
        ...
        clock_t start = clock();
        bool actual_result = sudoku_validator(sudoku);
        clock_t end = clock();
    ...
    printf("Total execution time using 3 threads: %.6f seconds\n", total_time);
}